...
function loginValidation() {
    ...
    if (LoginEmail.value !== "admin123@gmail.com") {
        alertReq("Required valid Email", LoginEmail);
        return false;
    }
    if (LoginPass.value !== "Admin@123") {
        alertReq("Required valid Password", LoginPass);
        return false;
    }
    alert("Login Success.");
...
Filechoose.addEventListener('change', function (event) {
    ...
    if (file.size > size) {
        ...
        ImgPreview.src = "";
        alertMsg("image size is too long! Choose image under 2MB.", "File");
        return;
    }
    ...
    if (!allowedTypes.includes(file.type)) {
        ...
        ImgPreview.style.display = "none";
        ImgPreview.src = "";
        alertMsg("a valid image (jpg, png, or gif).", "File");
        return;
    }
...
function validation() {
    ...
    if (ConfirmPas.value === "") {
        alertReq(`Required Confirm Password.`);
        return false;
    }
    ...
    if (Color.value === "#000000") {
        alertMsg("a Color other then black.", "Color");
        return;
    }
    ...
    if (Range.value === "0") {
        alertMsg("Rate from 1 to 10 except 0 number.", "Range");
        return;
    }
    ...
    if (Filechoose.value === "") {
        alertMsg("an image under 2 MB size.", "File");
        return;
    }
    //display alert for success register
...
function validate(value, fieldName, element, pattern = null) {
    ...
    if (value == "" || value.trim() == "") {
        alertReq(`Required ${fieldName}`, element);
        return false;
    }
    if (fieldName == "Address") {
        if (value.length < 5 || value.length > 60) {
            inputField.focus();
            alert(`${fieldName} must be less then 60 & greater then 5 character.`);
            return false;
        }
        return true;
        ...
        if (Number(value) < 16 || Number(value) > 100) {
            inputField.focus();
            alert(`${fieldName} must be less then 100 & greater then 16 Number.`);
            return false;
        }
        return true;
        ...
        if (selectedDate > today) {
            alert(`${fieldName} can not be future date`);
            inputField.focus();
            return false;
        } else if (selectedYear > 2006 || selectedYear < 2000) {
            alert(`${fieldName} Year must between 2000 to 2006.`);
            inputField.focus();
            return false;
        }
        return true;
        ...
        if (value.trim().length < 3) {
            alert(`${fieldName} at least 3 character.`);
            inputField.focus();
            return false;
        }
        return true;
    ...
    if (!pattern || !pattern.test(value.trim())) {
        if (fieldName == "Password") {
            alert(`Enter minimum 8 character long strong ${fieldName}.`);
            inputField.focus();
            return false;
        } else {
...
//validation for select types field(range,gender,hobbies,etc.)
function alertMsg(fieldName, element) {
    const inputField = document.getElementById(element);
    alert(`Select ${fieldName}`);
    inputField.focus();
}

//validation for first empty field
function alertReq(Msg, element) {
    const inputField = document.getElementById(element);
    alert(`${Msg}`);
    inputField.focus();
}
